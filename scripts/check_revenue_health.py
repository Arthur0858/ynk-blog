...
def clickthrough_links(links: list[dict[str, str]]) -> list[str]:
    ...
    for link in links:
        ...
        params = parse_qs(parsed.query)
        if path.endswith(EXPECTED_CLICKTHROUGH_PATH) and params.get("surface") and params.get("autoredirect") == ["1"]:
            rows.append(href)
    return rows
...
def build_report(out_dir: Path) -> dict[str, Any]:
    ...
    if not expected_home_gumroad_links:
        blockers.append("missing_home_gumroad_cta_with_utm")
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for key in ["product_gumroad_ctas", "home_gumroad_ctas", "home_product_page_ctas"]:
